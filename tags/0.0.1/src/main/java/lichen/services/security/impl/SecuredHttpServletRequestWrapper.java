 ...
 * file: $Id: SecuredHttpServletRequestWrapper.java 643 2008-09-22 06:30:41Z jcai $
...
public class SecuredHttpServletRequestWrapper implements HttpServletRequest {
	...
	private final SecurityPrincipalService _principalServie;
	private final String[] _roles;

	/**
	 * 
	 * @param request 原始的HttpServletRequest
	 * @param principalService 用于获取当前的登录用户,使用变量传递Pincipal会导致已经登录的用户状态在Immediate模式下无法获取
	 * @param roles
	 */
	SecuredHttpServletRequestWrapper(HttpServletRequest request, SecurityPrincipalService principalService,
			String[] roles) {
		this._request = request;
		this._principalServie = principalService;
		this._roles = roles;
	...
	public Principal getUserPrincipal() {
		return this._principalServie.getCurrentPrincipal();
	}
